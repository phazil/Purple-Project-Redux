...
This is my attempt to recreate Train's version, however there are differences. There is no custom wiring terminal pcb, it's not double stacked FR4 for both front/back panel, and there's no usage of low profile OSBMXs. I used a right angle header soldered onto a Basic Breakout Board running GP2040-CE along with a 20-pin wiring harness. There is some soldering involved in this to get the menu panel working however it is sequential and there's only 1 ground line needed. The single panel do not flex at all with the increase inside area of the 3D prints. you would have to try to push down really hard on the center of the panels to even see any sort of flex.
